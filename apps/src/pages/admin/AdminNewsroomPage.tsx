...
import { fetchNewsroomList, deleteNewsroom, type NewsroomItem } from '@/api/newsroom'

const PAGE_SIZE = 15

type NewsroomSearchParams = { keyword: string; type: number; date_from: string; date_to: string }
const defaultParams: NewsroomSearchParams = { keyword: '', type: 2, date_from: '', date_to: '' }
...
export default function AdminNoticePage() {
  ...
  const [searchParams, setSearchParams] = useState<NewsroomSearchParams>(defaultParams)
  ...
  const load = useCallback(async (p: number, params: NewsroomSearchParams) => {
    setLoading(true)
    setError(null)
    try {
      const res = await fetchNewsroomList({ page: p, size: PAGE_SIZE, ...params })
      setItems(res.items)
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await Promise.all(checkedIds.map((id) => deleteNewsroom(id)))
      load(page, searchParams)
    } catch (err: unknown) {
      alert(err instanceof Error ? err.message : '삭제에 실패했습니다.')
    }
        ...
        <main className="adm_main">
          <section className="adm_section">
            <div className="adm_toolbar">
              ...
              <button className="adm_btn_primary" onClick={() => navigate('/admin/newsroom/new')}>
                + 작성
              </button>
            </div>
                  ...
                  {loading ? (
                    ...
                    <tr>
                      ...
                      <tr key={item.id}>
                        ...
                        <td>
                          <Link to={`/admin/newsroom/${item.id}/edit`} className="adm_table_link">
                            {item.title}
                          </Link>
                        </td>
                        ...
                        <td className="adm_td_center">
                          <div className="adm_action_btns">
                            <button
                              ...
                              onClick={() => navigate(`/admin/newsroom/${item.id}/edit`)}
                            >
                              수정
                            </button>
                            <button
