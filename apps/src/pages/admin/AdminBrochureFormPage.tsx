...
const LABEL = '브로슈어'
const LIST_PATH = '/admin/brochure'
const CATEGORIES = ['브로슈어']

export default function AdminBrochureFormPage() {
  ...
  const [existingFiles, setExistingFiles] = useState<BrochureFile[]>([])
  ...
  const handleDeleteExistingFile = async (fileId: number) => {
    ...
    try {
      await deleteBrochureFile(fileId)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEdit) {
        await updateBrochure(Number(id), formData)
        alert('수정되었습니다.')
      } else {
